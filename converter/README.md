...
Konverter von **RobotSpec v0.2** nach **schema.org JSON-LD**. Reines ESM, keine
Laufzeit-Abhängigkeiten, Node 20+. Umsetzungsgrundlage ist die informative
Bridge [docs/bridge-schema-org.md](../docs/bridge-schema-org.md); normativ ist
...
|---|---|---|---|
| `doc` | `object` | — | RobotSpec-Listing v0.2 (validiert oder unvalidiert) |
| `options.profile` | `"merchant-listing" \| "product-snippet"` | `"merchant-listing"` | Ausgabeprofil, siehe unten |
| `options.vatMode` | `"gross" \| "net"` | `"gross"` | `gross` rechnet `vatRate` auf die Netto-Cent-Preise auf |
...
Weitere Exporte: `PROFILES`, `VAT_MODES` (die zulässigen Optionswerte) und ein
Default-Export, der auf `toSchemaOrg` zeigt.
...
- `Error`, wenn `doc.schemaVersion` gesetzt und ungleich `"0.2"` ist
...
Für Aggregatoren/Marktplätze. Die Angebote werden nach **Währung und
Abrechnungsperiode** gruppiert; je Gruppe entsteht ein `AggregateOffer` mit
`lowPrice`/`highPrice`/`offerCount`. Ein Kaufpreis landet dadurch nie mit einer
Monatsrate im selben Aggregat. Bei genau einer Gruppe ist `offers` ein Objekt,
sonst ein Array (Reihenfolge: Once, Jahr, Monat, Woche, Tag, je m²).
...
| `billingPeriod` | `UnitPriceSpecification.referenceQuantity` (`MON`/`DAY`/`WEE`/`ANN`/`MTK`) |
...
|---|---|
| Netto im `price`-Feld | Default `vatMode: "gross"`; der Nettobetrag bleibt ausschließlich in `rs:offer.priceCents`, `valueAddedTaxIncluded` wird passend gesetzt |
| Kauf- und Mietangebote in einem `AggregateOffer` | Gruppierung nach Währung **und** Abrechnungsperiode; `merchant-listing` erzeugt grundsätzlich kein `AggregateOffer` |
...
1. **Preise als Dezimalstring** (`"38068.10"` statt `38068.10`). JSON-Zahlen
   ...
   bei nicht ganzzahligen Sätzen wie CH 8,1 %.
...
8. **`priceComponentType: Subscription` nur bei `Rent`/`Leasing`/`RaaS`.**
   ...
   für Leasing wurde verworfen — die Bridge zeigt `Subscription`.
...
Die Golden-Files unter `converter/test/golden/` decken jedes v0.2-Listing-
...
GTIN, Pay-per-Use, CHF, PreOrder, gemischte Gruppen, Fehlerfälle), prüfen die
Unit-Tests in `converter/test.mjs`.

## Grenzen

- Der Konverter ist **kein Validator**. Ungültige Listings erzeugen ungültiges
  Markup; vorher `npm run validate` bzw. Ajv gegen
  `schema/robotspec-v0.2.schema.json` laufen lassen.
